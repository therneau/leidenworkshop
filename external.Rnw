...
(The hematologists' firmly believe that
physicians ``will not understand'' a continuous score.)

The 2002, 2012 and 2015 predictions differ substantially, particularly for
the lowest risk goup (there are over 50 subjects and 0 deaths in the top
2015 curve). 
This represents an era where several new and effective treatments for
...
no association but $C= 1/2$, plotting $(|R|+1)/2$ aligns the two.
...
As expected the dichotomized concordance is larger. The amount variation 
associated with small changes in $\tau$ is a consequece of the sharp transitions
in weight from $>1$ to 0 when a censored observation is reassigned.
Why is the DC variance grow so rapicly? As $\tau$ increases the RTTR algorithm
has set more and more observations' weight to zero: by time 6 only 33/387 remain.
