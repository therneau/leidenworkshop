...
\begin{enumerate}
 ...
 \item Create uncensored data specific to a chosen assessment time $\tau$,
    ...
    \begin{itemize}
      \item Use the redistribut-to-the-right (RTTR, IPC) algorithm to reassign 
        the 
  ...
  \item Ignore censoring. A disastrous approach, but one that sometimes 
    ...
    survival by comparing those dead before 5 to alive at 5 and simply
    ignoring all those censored before 5.
\end{enumerate}
...
<<gbsg2, fig.caption="Spline fit for number of nodes, Rotterdam">>=
...
(The hematologists firmly believe that a discrete score such as this
is much easier for the practioner to use.)

The 2004, 2012 and 2015 predictions differ substantially, particularly for
the lowest risk goup (there are 70 subjects and 0 deaths in the top
2015 curve). 
This data spans a time when several new and effective treatments for
amyloidosis became available.  Any validation exercise will need to be cognisant
of this.
...
as a predictor will reprise the KM.
The numerator $N$ is known as the Brier score. van Houwelingen and Putter
\cite{vanHouwelingen12} argue that $N$ is difficult to interpret without
rescaling, since
...
Cstat <- array(0, dim=c(151,2,6)) # value and std, 5 measures
for (i in 1:151) {
    ...
    c5 <- concordance(psmat[,i] ~ eta1, data=gbsg2, ymax=tau2[i])
...
\begin{itemize}
  \item For this particular data set pair, the discrimination of the model
    ...
    they differ by .01 to .02.
    At 4 years the values are .70 and .69, less than 1 std apart.
    (The blip of better performance around year 1.5 is an unexplained oddity.)
...
Now add in the dichotmized measures.  For R-squared, we note that $|R|=0$ for
no association while $C= 1/2$ for no association,
plotting $(|R|+1)/2$ aligns the two.
...
legend(2, .8, c("n(t) wt", "S/G weight", "DC", "(R+1)/2", "pseudovalue C"),
       lwd=2, lty=1, col=1:5, bty='n', cex=.8)
matplot(tau2, Cstat[,2,-6], lwd=2, lty=1, col=1:5, type='l', 
        ylim=c(0, max(Cstat[,2,])),
        xlab="Cutoff time tau", ylab="std(C)")
legend(1, .07, c("n(t) weight", "S/G weight", "DC", "Rotterdam", 
                 "pseudovalue C"),
       lwd=2, lty=1, col=1:5, bty='n')
par(opar)
@

As expected the dichotomized concordance is larger. The amount of variation 
associated with small changes in $\tau$ is, we presume,
 a consequece of the sharp transitions
...
of the censored subjects remain.
...
group.
A concordance based on pseudovalues uses all the pairs: two observations censored
at diffent times will have different pseudovalues (if at least one event
occurs in the interim).  The increase in the number of comparable pairs
leads to a lower estimated variance.
Due to its larger number of observations the estimated variance of $C$ for
the Rotterdam data is the smallest.
...
matplot(tau2, C2[,1,1:4], lwd=2, lty=1, col=1:4, type='l', ylim=c(.5, .86),
        xlab="Cutoff time tau", ylab="C")
...
lower, however, leading to far fewer comparaible pairs and an increase in the 
standard error.
...
Since several of the prediction models only extend to 60 months, use
this as a limit.
...
of those papers have chosen 5 years without presenting an explicit argument 
for that time point. 
...
In this case we lack any formal justfication for treating the last row below as
...
excess; one can argue that it is asking a discrimination rather
than a formal calibration question.  
(Said argument is a perhaps rather fussy semantics.)
...
\begin{itemize}
  ...
  \item Because the GBSG data set has subjects who are grade 1--3 while the 
    ...
    larger than the 2-3 increment. (Has model rfit2 has overestimated risk
...
The total deaths in the amloidosis validation data set at 5 years is 583, 
while expected
...
As stated earler, there were major gains in the treatment of AL amyloidosis
...
This leads to 1*log(0) in the Poisson loglikelihood for such subjects: observed
...
prediction model.
Statisticians often pick a scale where effects will additive and approximately
 constant over time (though if we are honest, the transform is normally 
chosen purely for convenience).
Both the complimentary log-log and logit have some success wrt this metric
