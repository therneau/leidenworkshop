...
The \code{amyloid.surv} data set contains digitized curves from the papers,
...
plot(survival~ month, ylim=c(0,1), lwd=2, data=amyloid.surv,
     type='l', subset= (study==2004 & stage==0))
for (i in 1:2)
    lines(survival ~ month, data=amyloid.surv, subset=(study==2004 & stage==i),
          col= i+1, lty=i+1, lwd=2)
text(40, .9, "2002", cex=1.5)

plot(survival~ month, ylim=c(0,1), lwd=2, data=amyloid.surv,
     type='l', subset= (study==2012 & stage==0))
for (i in 1:3)
    lines(survival ~ month, data=amyloid.surv, subset=(study==2012 & stage==i),
          col= i+1, lty=i+1, lwd=2)
text(40, .9, "2012", cex=1.5)

plot(survival~ month, ylim=c(0,1), lwd=2, data=amyloid.surv,
     type='l', subset= (study==2015 & stage==0))
for (i in 1:3)
    lines(survival ~ month, data=amyloid.surv, subset=(study==2015 & stage==i),
          col= i+1, lty=i+1, lwd=2)
text(40, .9, "2015", cex=1.5)

afit <- survfit(Surv(months, status) ~ number.organs, amyloid)
...
staging variables and/or cutoffs, but is not used in the final score. 
(The hematologist's in question firmly believe that their fellow
...
Estimating the coefficients for the risk score variables, de novo within this
...
\begin{itemize}
  \item For $C$ count ties in $t$ as uninformative, such pairs count as neither
  ...
  \item If the response is a 0/1 variable, then $C$ = AUROC, the area under the
    ...
    well known.)
  \item For survival data, pairs such as (5+, 7) as also treated as
...
Cstat <- array(0, dim=c(151,2,4)) # value and std, 4 measures
for (i in 1:151) {
    ...
    c3 <- concordance(temp~ eta1, data=gbsg2, weight=reweight[,i],
                      subset=(reweight[,i] > 0))
    c4 <- concordance(rfit1, ymax=tau[i]) # concordance of the training data
...
matplot(tau2, Cstat[,1,], lwd=2, lty=1, col=1:4, type='l', ylim=c(.6, .8),
        xlab="Cutoff time tau", ylab="C")
legend(1, .8, c("n(t) wt", "S/G weight", "DTC", "Rotterdam"),
...
matlines(tau2, Cstat[,1,1:3], lwd=1, lty=3, col='gray')
@ 
...
<<aconcord>>=
cfit <- concordance(Surv(months, status) ~ number.organs + r2004 + r2012 + 
...
cat(i,stage, "  ")
        amexpect[indx1, i] <- efun(amyloid$months[indx1], tdata$month[indx2],
...
<<amsurv1>>=
asurv15 <- survfit(Surv(months, status) ~ r2015, amyloid)
...
<<amsurv2>>=
asurv12 <- survfit(Surv(months, status) ~ r2012, amyloid)
plot(asurv15, col=1:4, lwd=2, xlab="Months", ylab="Survival")
for (i in 0:3) {
